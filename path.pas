...
// NPind returns a pointer to the node with the given index.
// It takes a pointer for performance reasons; the node pool is never modified.
function NPind(np: pNodePool; ind: int): pWaypoint; inline;
begin
	exit(@(np^.nodes[ind]));
end;
...
procedure PQinsert(pq: pPointQueue; point: int; all: pNodePool);
var
	where, i: int;
begin
	where := pq^.first;
...
procedure PQadd(pq: pPointQueue; point: int; all: pNodePool);
var
	newlen: int;
begin
